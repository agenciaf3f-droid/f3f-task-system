...
import { ArrowLeft, Plus, Loader2, AlertCircle, X, ChevronDown, ChevronRight } from "lucide-react";
...
export function EditTemplateForm({ templateId, initialData, initialTasks, sectors, users }: EditTemplateFormProps) {
  ...
  const [expandedTasks, setExpandedTasks] = useState<Set<string>>(
    () => new Set(initialTasks.filter((t) => t.subtasks.length > 0).map((t) => t.id)),
  );
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-neutral-900">Editar template</h1>
      </div>
                  ...
                  <div className="flex items-center gap-2 px-3 pb-3 pl-10">
                    ...
                    >
                      ...
                      {task.subtasks.length > 0
                        ? `${task.subtasks.length} subtarefa${task.subtasks.length !== 1 ? "s" : ""}`
                        : "Subtarefas"}
                    </button>
                  </div>
